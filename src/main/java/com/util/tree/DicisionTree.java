...
public class DicisionTree {
	...
	public static void main(String[] args) throws Exception {
		...
		// 生成决策树
		Tree decisionTree = (Tree) generateDecisionTree(samples, attrNames);
		Map<String, String> te = new HashMap<>();
		te.put("AGE", ">40");
		te.put("INCOME", "Medium");
		te.put("STUDENT", "Yes");
//		/Tree tt = decisionTree;
		decideResult(decisionTree, te);
		System.out.println(te.get("result"));
	 ...
	 */
	public static  Map<Object, List<Sample>> readSamples(String[] attrNames,Object[][] rawData) {
	 ...
	 * 构造决策树
	 */
	public static Object generateDecisionTree(Map<Object, List<Sample>> categoryToSamples, String[] attrNames) {
	...
	public static void decideResult(Object tt, Map ob) {
		int result=0;
		if(tt instanceof Tree) {
			Tree tree = (Tree) tt;		
		for(Object attrValue:tree.getAttributeValues()){
		if(ob.get(tree.getAttribute()).equals(attrValue)){
			Object child = tree.getChild(attrValue);
			//System.out.println(tree.getAttribute());
			decideResult(child,ob);
		}
		}
		}
		else {
			ob.put("result",tt.toString());
		}
	}
